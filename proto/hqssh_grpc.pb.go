...
type SessionServiceClient interface {
	...
	// Watch session events across all sessions (bell rung, session ended).
...
func (c *sessionServiceClient) WatchEvents(ctx context.Context, in *WatchEventsRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[SessionEvent], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &SessionService_ServiceDesc.Streams[3], SessionService_WatchEvents_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
...
func _SessionService_WatchEvents_Handler(srv interface{}, stream grpc.ServerStream) error {
	m := new(WatchEventsRequest)
	if err := stream.RecvMsg(m); err != nil {
		return err
	}
	return srv.(SessionServiceServer).WatchEvents(m, &grpc.GenericServerStream[WatchEventsRequest, SessionEvent]{ServerStream: stream})
}
...
var SessionService_ServiceDesc = grpc.ServiceDesc{
	...
	Streams: []grpc.StreamDesc{
		...
		},
		{
			...
			Handler:       _SessionService_WatchEvents_Handler,
			ServerStreams: true,
		},
	},
	Metadata: "proto/hqssh.proto",
